    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(new QShortcut(QKeySequence(Qt::Key_M), this), &QShortcut::activated, this, &MainWindow::move_img);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::Key_M), this), &QShortcut::activated, this, &MainWindow::select_move_dir);
    ...
    QSettings sessionSettings("YoloLabel", "Session");
    m_moveDir = sessionSettings.value("moveDir").toString();
...
void MainWindow::restoreLastSession()
{
    ...
    m_moveDir = s.value("moveDir").toString();
...
bool MainWindow::move_file_with_fallback(const QString& src, const QString& dst) const
{
    if(src.isEmpty() || !QFile::exists(src)) return true;
...
void MainWindow::select_move_dir()
{
    QString startDir = m_moveDir.isEmpty() ? m_imgDir : m_moveDir;
    QString selectedDir = QFileDialog::getExistingDirectory(this, tr("Select Move Directory"), startDir);

    if(selectedDir.isEmpty()) return;

    m_moveDir = selectedDir;
    saveSession();
    statusBar()->showMessage("Move directory set: " + m_moveDir, 5000);
}
...
void MainWindow::move_img()
{
    ...
    if(m_moveDir.isEmpty() || !QDir(m_moveDir).exists())
    {
        select_move_dir();
        if(m_moveDir.isEmpty() || !QDir(m_moveDir).exists()) return;
    }

    if(ui->label_image->isOpened()) save_label_data();

    const QString srcImgPath = m_imgList.at(m_imgIndex);
    const QString srcLabelPath = get_labeling_data(srcImgPath);

    QFileInfo imgInfo(srcImgPath);
    QDir moveDir(m_moveDir);

    QString imgSuffix = imgInfo.suffix();
    QString targetBase = imgInfo.completeBaseName();
    QString candidateBase = targetBase;
    QString dstImgPath;
    QString dstLabelPath;

    int idx = 0;
    do
    {
        candidateBase = (idx == 0) ? targetBase : targetBase + "_" + QString::number(idx);
        dstImgPath = moveDir.filePath(candidateBase + (imgSuffix.isEmpty() ? "" : "." + imgSuffix));
        dstLabelPath = moveDir.filePath(candidateBase + ".txt");
        idx++;
    } while(QFile::exists(dstImgPath) || QFile::exists(dstLabelPath));

    bool movedImg = move_file_with_fallback(srcImgPath, dstImgPath);
    bool movedLabel = true;
    if(QFile::exists(srcLabelPath))
        movedLabel = move_file_with_fallback(srcLabelPath, dstLabelPath);

    if(!movedImg)
    {
        pjreddie_style_msgBox(QMessageBox::Warning, "Move failed", "Failed to move the current image file.");
        return;
    }

    if(!movedLabel)
    {
        pjreddie_style_msgBox(QMessageBox::Warning, "Move warning", "The image was moved, but the matching TXT label file could not be moved.");
    }
    ...
    goto_img(m_imgIndex);
    statusBar()->showMessage("Moved image and label to: " + m_moveDir, 5000);
}
